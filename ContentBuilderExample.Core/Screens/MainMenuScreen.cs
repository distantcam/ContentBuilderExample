...
using ContentBuilderExample.Core;
using ContentBuilderExample.Core.Effects;
using ContentBuilderExample.Core.Inputs;
...
namespace ContentBuilderExample.Screens;
...
class MainMenuScreen : MenuScreen
{
    ...
    private void SetLanguageText()
    {
        ...
        Title = "ContentBuilderExample"; // TODO uncomment this if you want it to use Resources.MainMenu; instead
    }
    ...
    public override void LoadContent()
    {
        ...
        settingsManager ??= ScreenManager.Game.Services.GetService<SettingsManager<ContentBuilderExampleSettings>>();
    ...
    private void UpdateTutorialSteps(GameTime gameTime)
    {
        if (gameTime.TotalGameTime - timeSinceLastMessage > TimeSpan.FromSeconds(3)) // Should the showtime be in settings?
        {
            ...
            if (ContentBuilderExampleGame.IsMobile)
            {
                if (tutorialStep > 3)
                {
                    tutorialStep = -1;
